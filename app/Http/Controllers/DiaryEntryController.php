...
class DiaryEntryController extends Controller
{
    ...
    public function index()
    {
        ...
        // Query to find entries that are Sad + mention happy
        // $conflictingEntries = DB::table('diary_entries as de')
        //     ->join('diary_entry_emotions as dee', 'de.id', '=', 'dee.diary_entry_id')
        //     ->where('dee.emotion_id', 2) // Sad (emotion id = 2)
        //     ->where('de.user_id', $userId)
        //     ->where('de.content', 'like', '%happy%')
        //     ->select('de.*')
        //     ->get();

        // Return the view with all diary entries, sad-but-happy and summary data
        //return view('diary.index', compact('diaryEntries', 'summary', 'conflictingEntries'));
    ...
    public function getConflict()
    {
        ...
        $conflictingEntries = DB::table('diary_entries as de')
            ...
            ->where('de.user_id', $userId)
            ->where('de.content', 'like', '%happy%')
            ->select('de.id', 'de.date', 'de.content', 'e.name as emotion', 'dee.intensity')
            ->orderBy('de.date', 'desc')
            ->get();

        return view('getConflict.index', compact('conflictingEntries'));
    }
